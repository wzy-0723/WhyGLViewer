    ...
    void ParseGLTFNode(cgltf_node* node, GameObject* parent, const std::filesystem::path& folder)
    {
        ...
        if (node->mesh)
        {
            for (cgltf_size pi = 0; pi < node->mesh->primitives_count; ++pi)
            {
                ...
                // 需要一个默认材质，以防图形没有材质
                mat->SetShaderProgram(SINGLETON_PTR(GraphicsAPI)->GetDefaultShaderProgram());


                // gltf材质是PBR，两种工作流程：金属粗糙度metallic_roughness和镜面光泽度specular_glossiness
